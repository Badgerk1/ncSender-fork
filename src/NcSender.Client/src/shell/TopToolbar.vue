  ...
  <div class="toolbar" :class="statusClass">
    <div class="toolbar__left">
      ...
      <div class="logo-container">
        <span
          class="logo"
          @mousedown="startKioskExitPress"
          @mouseup="cancelKioskExitPress"
          @mouseleave="cancelKioskExitPress"
          @touchstart.prevent="startKioskExitPress"
...
// Probe pin state: Pn:P (server normalizes across protocols)
const isProbeTriggered = computed(() => {
  const pnString = store.status.Pn || '';
  return pnString.includes('P');
});

// TLS pin state: Pn:T (server normalizes grblHAL P:1 → Pn:T)
const isTLSTriggered = computed(() => {
  const pnString = store.status.Pn || '';
  return pnString.includes('T');
});
...
// Kiosk exit long-press (5 seconds)
const KIOSK_EXIT_HOLD_MS = 5000;
let kioskExitTimer: ReturnType<typeof setTimeout> | null = null;

const startKioskExitPress = () => {
  if (kioskExitTimer) return;
  kioskExitTimer = setTimeout(() => {
    kioskExitTimer = null;
    // Check if running in Electron with ncSender API
    const ncSenderApi = (window as any).ncSender;
    if (ncSenderApi?.app?.quit) {
      ncSenderApi.app.quit();
    }
  }, KIOSK_EXIT_HOLD_MS);
};

const cancelKioskExitPress = () => {
  if (kioskExitTimer) {
    clearTimeout(kioskExitTimer);
    kioskExitTimer = null;
  }
};
